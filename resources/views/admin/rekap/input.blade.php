...
@section('content')
  ...
  <section x-data="omsetForm()" class="mt-6 bg-white rounded-2xl shadow p-5">
            ...
            <tr class="border-t">
              ...
              <td class="px-3 py-2">
                <div class="flex items-center justify-center gap-2">
                  <button type="button" class="h-7 w-7 rounded border" @click="r.qty = Math.max(1, r.qty-1)">–</button>
                  <input type="number" min="1" class="w-16 text-center border rounded px-2 py-1"
                         :name="`rows[${idx}][qty]`" x-model.number="r.qty">
                  <button type="button" class="h-7 w-7 rounded border" @click="r.qty++">+</button>
                </div>
              </td>
              <td class="px-3 py-2">
                <input type="number" class="w-28 text-right border rounded px-2 py-1"
                       :name="`rows[${idx}][subtotal]`" x-model.number="r.subtotal">
              </td>
              ...
              <td class="px-3 py-2 text-center">
                <select class="border rounded px-2 py-1"
                        :name="`rows[${idx}][kategori]`" x-model="r.kategori">
                  <option value="Lunas">Lunas</option>
                  <option value="Bayar Bon">Bayar Bon</option>
    ...
    <div class="mt-3 flex items-center justify-between">
      ...
      <form method="POST" action="{{ route('admin.rekap.store') }}">
        @csrf
        {{-- sinkronisasikan rows Alpine ke input hidden --}}
        <template x-for="(r,idx) in rows" :key="'h'+idx">
          <div class="hidden">
            ...
            <input type="hidden" :name="`rows[${idx}][subtotal]`" :value="r.subtotal">
            <input type="hidden" :name="`rows[${idx}][metode_pembayaran_id]`" :value="r.metode_pembayaran_id">
            <input type="hidden" :name="`rows[${idx}][kategori]`" :value="r.kategori">
            <input type="hidden" :name="`rows[${idx}][total]`" :value="r.qty * r.subtotal">
          </div>
        </template>
  ...
  <section class="mt-6 bg-white rounded-2xl shadow p-5">
    ...
    <form method="POST" action="{{ route('admin.rekap.store-saldo') }}" class="grid md:grid-cols-3 gap-4">
      ...
      <div>
        ...
        <input name="saldo_kartu" type="number" class="w-full border rounded px-3 py-2" value="{{ old('saldo_kartu',0) }}">
      </div>
      <div>
        <label class="text-sm">Jumlah Tap Gagal</label>
        <input name="tap_gagal" type="number" class="w-full border rounded px-3 py-2" value="{{ old('tap_gagal',0) }}">
      </div>
  ...
  <script>
    function omsetForm() {
      return {
        rows: [{service_id:'', qty:1, subtotal:0, metode_pembayaran_id:'{{ optional($metodes->first())->id }}', kategori:'Lunas'}],
        add(){ this.rows.push({service_id:'', qty:1, subtotal:0, metode_pembayaran_id:'{{ optional($metodes->first())->id }}', kategori:'Lunas'}); },
        remove(i){ this.rows.splice(i,1); }
      }
    }
    function pengeluaranForm() {
      return {
        rows: [{keterangan:'', subtotal:0, tanggal:new Date().toISOString().slice(0,10), metode_pembayaran_id:'{{ optional($metodes->first())->id }}'}],
        add(){ this.rows.push({keterangan:'', subtotal:0, tanggal:new Date().toISOString().slice(0,10), metode_pembayaran_id:'{{ optional($metodes->first())->id }}'}); },
        remove(i){ this.rows.splice(i,1); }
      }
    }
  </script>
